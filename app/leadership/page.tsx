              ...
              {executives.map((exec, index) => (
                ...
                >
                  ...
                  <CardHeader className="space-y-2">
                    <CardTitle className="font-display text-2xl">{exec.name}</CardTitle>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button variant="outline" size="sm" className="w-full group/btn bg-transparent" asChild>
                      <a href={`mailto:${exec.email}`} className="flex items-center justify-center gap-2">
                        <Mail className="w-4 h-4" />
                        <span className="text-xs truncate">{exec.email}</span>
                      </a>
                    </Button>
                  </CardContent>
                </Card>
        ...
        <section className="py-20 px-4 bg-gradient-to-b from-muted to-background">
          <div className="container mx-auto max-w-3xl">
            <Card className="text-center shadow-xl border-2 border-accent/20">
              <CardHeader>
                <CardTitle className="font-display text-4xl md:text-5xl mb-4">JOIN OUR TEAM?</CardTitle>
                <CardDescription className="text-base leading-relaxed">
                  Elections happen each spring, and all members are welcome to run for positions. We're always looking
                  for passionate birders to help lead the club!
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="relative inline-block">
                  <Button size="lg" onClick={copyEmail} className="px-8">
